...
contract Domains is ERC721URIStorage {
    ...
    function price (string calldata name) public pure returns(uint256) {
        ...
        require(len > 0, "Invalid domain name Bro!");
        if (len == 3) {
            return 5 * 10**17;
        }
        else if (len == 4) {
    ...
    function register(string calldata name) public payable {
        ...
        //combine the name passed into the function and the tld
        string memory _name = string(abi.encodePacked(name, ".", tld));
        // Create image for the nft with the same name
        ...
        bytes(
            ...
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Ace name service", "image": "data:image/svg+xml;base64,',
